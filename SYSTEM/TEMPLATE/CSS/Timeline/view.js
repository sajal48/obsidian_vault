			...
			try {
				...
				if (abstractFilePath) {
					app.vault.read(abstractFilePath).then(function(fileText) {
						const sectionHeader = input.section;  // Dynamically set the section header based on the parameter
						const taskToInsert = "- [ ] #task " + newTask;  // Format your task
					
						// Check if the section exists in the file
						if (fileText.includes(sectionHeader)) {
							// Find the position where the section starts
							const sectionIndex = fileText.indexOf(sectionHeader);
							
							// Find the end of the section header line
							const afterSectionIndex = fileText.indexOf("\n", sectionIndex) + 1;
					
							// Find the start of the next section (next header like `# Overview`) or the end of the file
							const nextSectionIndex = fileText.indexOf("\n#", afterSectionIndex);  // Find the next header (e.g., `# Overview`)
							const sectionEndIndex = nextSectionIndex === -1 ? fileText.length : nextSectionIndex;
					
							// Get the content of the section
							let sectionContent = fileText.slice(afterSectionIndex, sectionEndIndex).trimEnd();  // Trim any trailing blank lines
					
							// Add the new task after the last content of the section
							const newFileText = fileText.slice(0, afterSectionIndex) + sectionContent + "\n" + taskToInsert + "\n\n" + fileText.slice(sectionEndIndex);
					
							// Update the file with the new content
							app.vault.modify(abstractFilePath, newFileText);
					
						} else {
							// If the section doesn't exist, append at the end of the file
